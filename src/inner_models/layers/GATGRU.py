...
from src.inner_models.layers.GAT import GATNet
...
class GAT_GRUaaa(nn.Module):
    ...
    def norm(self, A, device, add=True):
        if add == False:
    ...
    def forward(self, X, A,device):
        for i in range(self.num_channels):
            data = {"flow_x": X, "graph": self.norm(A[:, i, :, :],device)}
            if i == 0:
                X_ = self.gat_layer(data,device) * self.soft[i]
            else:
                X_ = X_ + self.gat_layer(data,device) * self.soft[i]
...
class GAT_GRU(nn.Module):
    def __init__(self, in_features, num_f, dim_with_c, num_channel):
        super(GAT_GRU, self).__init__()
        self.num_f = num_f
        self.in_dim = 20
        self.out_dim = 20
        ...
        self.gat_layer = GATNet(in_c=in_features, hid_c=self.out_dim, out_c=20, n_heads=6)
        self.weight = nn.Parameter(torch.Tensor(1, 1))
        self.soft = nn.Parameter(torch.Tensor(self.num_channels))
        self.reset_parameters()
    ...
    def forward_old(self, X, A,device):
        for i in range(self.num_channels):
            data = {"flow_x": X, "graph": self.norm(A[:, i, :, :],device)}
            if i == 0:
                X_ = self.gat_layer(data,device)
                X_ = X_ * self.soft[i]
            else:
                X_tmp = self.gat_layer(data,device)
    ...
    def forward(self, X, A, device):
        for i in range(self.num_channels):
            data = {"flow_x": X, "graph": self.norm(A[:, i], device)}
            out = self.gat_layer(data, device) * self.soft[i]
            X_ = out if i == 0 else X_ + out

        # GRU over nodes, KEEP sequence
        #X_ = X_.transpose(1, 2)        # [Bθ, N, F]
        X_out, _ = self.gru(X_)        # [Bθ, N, F]
        X_out = self.linear(X_out)     # [Bθ, N, F]
