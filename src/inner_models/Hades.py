...
from src.inner_models.layers.Hades.utils import Embedder, Decoder
from src.inner_models.layers.Hades.kpi_model import KpiEncoder
from src.inner_models.layers.Hades.log_model import LogEncoder




class HadesWrapper(nn.Module):
    def __init__(self,
                 raw_metric, raw_logs, 
                 feature_metric, feature_logs, device):
        super().__init__()
        ...
        kwargs = {
            ...
            ### Kpi params
            "kpi_architect":"by_metric",#"by_aspect",
            "temporal_kernel_sizes":[2,2],  
            "temporal_hidden_sizes":[64,4],
            "temporal_dropout":0,
            "pooling":True,

            # Inner params
            "inner_hidden_sizes":[256,256,256],
            "inner_kernel_sizes":[3,3,3],
            "inner_dropout":0.5,
        ...
        self.model = MultiModel(var_nums=feature_metric, vocab_size=feature_logs, device=device, **kwargs)
    ...
    def forward(self, metrics, logs):
        ...
        #features = torch.cat([m, L], dim=-1)
        features = {"kpi_features": m, "log_features": L}
        rec = self.model(features)
