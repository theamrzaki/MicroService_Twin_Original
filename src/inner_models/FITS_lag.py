...
def laguerre_encode(input_seq, degree=64):
    B, T, C = input_seq.shape
    input_seq_flat = input_seq.reshape(B * C, T).T  # [T, B*C]
    device = input_seq.device
    coeffs = laguerre_torch(input_seq_flat, degree - 1, rtn_data=False, device=device)
    return coeffs.reshape(B, C, degree).to(device)

def laguerre_decode(coeffs, seq_len):
    B, C, D = coeffs.shape
    coeffs_flat = coeffs.reshape(B * C, D)

    # Evaluate Laguerre basis
    tvals = np.linspace(0, 5, seq_len)
    laguerre_polys = np.array([La.basis(i)(tvals) for i in range(D)])  # [D, T]
    laguerre_polys = torch.from_numpy(laguerre_polys).float().to(coeffs.device)

    # Reconstruct signal
    recon = torch.mm(coeffs_flat, laguerre_polys)  # [B*C, T]
    recon = recon.reshape(B, C, seq_len).permute(0, 2, 1)  # [B, T, C]
    return recon
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        specx = laguerre_encode(x, degree=2)  # tune degree if needed
        ...
        low_xy = laguerre_decode(full_spec.permute(0, 2, 1), seq_len=self.seq_len )
