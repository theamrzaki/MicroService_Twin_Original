...
class Process:
    ...
    def _load_csv_dir_metric(self, path):
        """Load and merge all CSVs under a directory (recursively)."""
        if not os.path.exists(path):
            logging.warning(f"Path not found: {path}")
            return pd.DataFrame()
        ...
        # Generator keeps memory usage reasonable
        return pd.concat((pd.read_csv(f) for f in csv_files), ignore_index=True)


    def __select_top_kpis__(self, metric_df: pd.DataFrame, gt_df: pd.DataFrame, threshold=0.95):
        """
        Dynamically selects impactful KPIs that contribute to the top 'threshold' 
        of total variance during anomaly windows.
        """
        import numpy as np
        print(f"Dynamically selecting impactful KPIs ...")

        # 1. Map timestamps to anomaly status
        #is_anomaly_mask = pd.Series(False, index=metric_df['timestamp'].unique())
        #for _, row in gt_df.iterrows():
        #    if row['timestamp'] in is_anomaly_mask.index:
        #        is_anomaly_mask.loc[row['timestamp']] = True

        # window size in the same unit as timestamps (e.g., seconds)
        window = 600  # example: ±5 minutes

        is_anomaly_mask = pd.Series(False, index=metric_df['timestamp'].unique())
        is_anomaly_mask = is_anomaly_mask.sort_index()

        for _, row in gt_df.iterrows():
            ts = row['timestamp']
            if ts not in is_anomaly_mask.index:
                continue

            is_anomaly_mask.loc[
                (is_anomaly_mask.index >= ts - window) &
                (is_anomaly_mask.index <= ts + window)
            ] = True


        anomaly_timestamps = is_anomaly_mask[is_anomaly_mask].index
        self.kpi_list = metric_df['kpi_name'].unique().tolist()
        print(f"Initial KPI count: {len(self.kpi_list)}")
        # 2. Score KPIs based on Anomaly-Window Variance
        kpi_scores = []
        for kpi in self.kpi_list:
            kpi_data = metric_df[metric_df['kpi_name'] == kpi]
            if kpi_data.empty:
                continue
                
            # Focus only on values during known failures
            anomaly_vals = kpi_data[kpi_data['timestamp'].isin(anomaly_timestamps)]['value']
            
            if len(anomaly_vals) > 1:
                variance = anomaly_vals.var()
                if variance > 1e-6: # Ignore absolute zeros/constant metrics
                    kpi_scores.append((kpi, variance))

        # 3. Calculate Cumulative Importance
        # Sort by variance descending
        kpi_scores.sort(key=lambda x: x[1], reverse=True)
        
        total_variance = sum(score for _, score in kpi_scores)
        if total_variance == 0:
            print("Warning: No variance found in any KPIs during anomalies. Keeping all.")
            return

        # 4. Select top KPIs that reach the threshold (e.g., 90% of total system variance)
        num_of_kpis_to_select = 22
        selected_kpis = []
        current_variance_sum = 0
        i = 0
        for kpi, score in kpi_scores:
            if i >= num_of_kpis_to_select:
                break
            selected_kpis.append(kpi)
            i += 1
        # Update class variables
        self.kpi_list = sorted(selected_kpis)
    ...
    def load_raw(self):
        ...
        for i, date_dir in tqdm(enumerate(dates), desc="Processing days"):
            ...
            df_gt["end"] = df_gt["end"].astype(int)


            #---------------- Metrics ----------------#
            # 1. Load Metricsself._load_csv_dir(os.path.join(day_path, 'metric'))
            #df_metric_1 = self._load_csv_dir(os.path.join(day_path, 'metric/container'))
            df_metric = self._load_csv_dir(os.path.join(day_path, 'metric/container'))
            #df_metric = pd.concat([df_metric_1, df_metric_2], ignore_index=True)
...
#
            ...
            tensor_envoy = self.densify_log_features(sparse_envoy, df_metric["data"].shape[0], num_services, max_templates=100)
            tensor_service = self.densify_log_features(sparse_service, df_metric["data"].shape[0], num_services, max_templates=100)
#
    ...
    def process_logs_with_drain(self, file_path, service_map, time_map, existing_miner=None):
        ...
        if existing_miner is not None:
            ...
            # Core fixes
            config.drain_st = 0.5            # ↓ similarity threshold (merge more)
        ...
        for chunk in tqdm(pd.read_csv(file_path, chunksize=500000),
            ...
            # 1. Prepare a sorted array of master timestamps
            master_ts = np.array(sorted(time_map.keys()))

            # 2. Instead of: chunk['t_idx'] = chunk['timestamp'].map(time_map)
            # Use searchsorted to find the closest master timestamp for every row in the chunk
            def fuzzy_map_timestamps(chunk_ts, master_ts, tolerance=300):
                # Find the insertion index to maintain order
                ...
                # Create a mask for hits within our 'window' (tolerance)
                valid_mask = diffs <= tolerance
                
                # Return the index from the time_map for the matched master timestamp
                # Return -1 or NaN for those outside the tolerance window
                matched_ts = master_ts[indices]
                return np.where(valid_mask, matched_ts, np.nan)

            # Applying it in your loop:
            matched_times = fuzzy_map_timestamps(chunk['timestamp'].values, master_ts)
